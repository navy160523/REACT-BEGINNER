...
export function AppFileUpload({ file, onChange }: Props) {
  ...
  const handleChangeFile = (event:React.ChangeEvent<HTMLInputElement>) => {
    ...
    //동일 파일 선택이 불가능 할 수 있으므로 event.target.value를 초기화
    event.target.value = "";
  }
  ...
  const handleRenderPreview = () => {
    if (typeof file === "string") {
      return <img src={file} alt="@THUMBNAIL" className="w-full aspcect-video rounded-lg object-cover border" />;
    } else if (file instanceof File) {
      return <img src={URL.createObjectURL(file)} alt="@THUMBNAIL" className="w-full aspcect-video rounded-lg object-cover border" />;
    }
